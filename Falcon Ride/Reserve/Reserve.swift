                        ...
                        LazyVStack(spacing: 10) {
                            if isLoading {
                                ...
                                ForEach(filteredRides()) { ride in
                                    NavigationLink(destination: OtherUserProfile(rideInfo: .reserve(ride), additionalInfo: ride.additionalInfo ?? "", fromLocation: ride.fromLocation, toLocation: ride.toLocation, time: ride.time, seats: ride.seats, donationRequested: ride.donationRequested ?? "")) {
                                        RideCell(ride: ride, width: geometry.size.width - 20, height: geometry.size.width * 0.5, onDelete: { selectedRide in
    ...
    func fetchRides() {
        ...
        ridesRef.observe(.value) { snapshot in
            ...
            group.notify(queue: .main) {
                // Sort the rides by date and time in descending order
